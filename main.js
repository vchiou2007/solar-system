/**
 * 3D Solar System v2.1 — All controls verified
 * Three.js module — GitHub Pages ready
 */
...
function mkStars() {
    ...
    for (let i = 0; i < n; i++) {
        const r = 80 + Math.random() * 120, th = Math.random() * Math.PI * 2, ph = Math.acos(2 * Math.random() - 1);
        p[i * 3] = r * Math.sin(ph) * Math.cos(th); p[i * 3 + 1] = r * Math.sin(ph) * Math.sin(th); p[i * 3 + 2] = r * Math.cos(ph);
        const q = Math.random();
        if (q < 0.08) { c[i * 3] = 0.7; c[i * 3 + 1] = 0.8; c[i * 3 + 2] = 1.0; }
        else if (q < 0.15) { c[i * 3] = 1.0; c[i * 3 + 1] = 0.85; c[i * 3 + 2] = 0.6; }
        else { const b = 0.5 + Math.random() * 0.5; c[i * 3] = b; c[i * 3 + 1] = b; c[i * 3 + 2] = b; }
    }
    ...
    const stars = new THREE.Points(g, new THREE.PointsMaterial({ size: 0.15, vertexColors: true, blending: THREE.AdditiveBlending, depthWrite: false }));
...
// ── SUN ──
const sunTex = mkTex(512, 256, (ctx, w, h) => {
    const g = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, w / 2);
    g.addColorStop(0, '#ffffff'); g.addColorStop(0.08, '#fffde0'); g.addColorStop(0.25, '#ffe040'); g.addColorStop(0.5, '#ff9900'); g.addColorStop(0.75, '#ee5500'); g.addColorStop(0.9, '#cc2200'); g.addColorStop(1, '#881100');
    ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
    for (let i = 0; i < 50; i++) { const sx = Math.random() * w, sy = Math.random() * h, sr = 3 + Math.random() * 14; const sg = ctx.createRadialGradient(sx, sy, 0, sx, sy, sr); sg.addColorStop(0, 'rgba(255,200,50,0.7)'); sg.addColorStop(0.5, 'rgba(255,120,20,0.4)'); sg.addColorStop(1, 'rgba(255,60,0,0)'); ctx.fillStyle = sg; ctx.beginPath(); ctx.arc(sx, sy, sr, 0, Math.PI * 2); ctx.fill(); }
});
...
planetDefs.forEach((def, idx) => {
    ...
    scene.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts), new THREE.LineBasicMaterial({ color: orbitColors[idx], transparent: true, opacity: 0.55 })));
...
moon.name = 'Moon'; moon.userData = { orbR: 2.0, sp: 7.5, ang: 0, stdMat: moonStd, basicMat: moonBasic }; scene.add(moon);
...
earth.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints(mOpts), new THREE.LineBasicMaterial({ color: 0x556688, transparent: true, opacity: 0.3 })));
...
function setLightingMode(realistic) {
    ...
    console.log('Lighting:', realistic ? 'realistic' : 'full bright');
}
...
function animate() {
    ...
    planets.forEach(p => { const u = p.userData; u.ang += edt * u.sp * 0.5; p.position.x = Math.cos(u.ang) * u.orbR; p.position.z = Math.sin(u.ang) * u.orbR; p.rotation.y += edt * u.rotSp; p.children.forEach(c => { if (c.userData && c.userData.isLabel) c.visible = state.showLabels; }); });
    moon.userData.ang += edt * moon.userData.sp * 0.5; moon.position.x = earth.position.x + Math.cos(moon.userData.ang) * moon.userData.orbR; moon.position.z = earth.position.z + Math.sin(moon.userData.ang) * moon.userData.orbR; moon.rotation.y += edt * 0.01; moonLabel.visible = state.showLabels;
    starfield.rotation.y += edt * 0.002; starfield.rotation.x += edt * 0.001; asteroidBelt.rotation.y += edt * 0.03;
...
// ── START ──
animate();
$('info-tooltip').textContent = '🖱️ 拖曳旋轉 | 🔍 滾輪縮放 | ⌨️ 空白:暫停 K:光影 B:背景 L:標籤';
$('info-tooltip').classList.remove('hidden');
setTimeout(() => $('info-tooltip').classList.add('hidden'), 7000);
console.log('🪐 Solar System v2.1 ready — all controls active');
console.log('Pause=Space | Lighting=K | Background=B | Labels=L | Top=T | Reset=R');
